...
public class Minkowski {
	 ...
	 */
	public static List<List<Edge>> generateMinkowskiNFP(MultiPolygon polyA, MultiPolygon polyB) throws FileNotFoundException {
...
//		//algorithm 3
	...
	private static List<Edge> MinkPos(List<Edge> qList, List<Edge> rList) {
		...
		while(!mergeList.get(mergeListStartPosition).isPolygonA() 
				|| mergeList.get(mergeListStartPosition).getEdgeNumber() 
				!= qList.get(0).getEdgeNumber())
		{
	...
	private static List<Edge> MinkNeg(List<Edge> aList, List<Edge> bList) {
		...
		while(bCount>0 || j<sList.size()-1){
			...
			if(si.isPolygonA()){
				...
				next = 0;
				if(si.getEdgeNumber() == direction * rList.get(next).getEdgeNumber()){
					j = j+1;
					bCount--;
					seqList.add(si);
					next = next+direction;
