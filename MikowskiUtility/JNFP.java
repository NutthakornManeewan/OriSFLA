...
public class JNFP {
	 ...
	 * @throws FileNotFoundException when the file is not found
	 */
	public static List<List<Edge>> GenerateMinkowskiNFP(File polyAFile, File polyBFile) throws FileNotFoundException{
		Minkowski.adjustRound(1);
		MultiPolygon polyA = new MultiPolygon(polyAFile);
		MultiPolygon polyB = new MultiPolygon(polyBFile);
		
		List<List<Edge>> nfp = Minkowski.generateMinkowskiNFP(polyA, polyB);
		return nfp;
	}
